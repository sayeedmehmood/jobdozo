    ...
    <>
      ...
      <aside className={cn("sidebar employer-sidebar", mobileOpen && "open")}>
        <Link href="/dashboard" className="sidebar-brand" onClick={onClose}>
          <span className="brand-icon"><img src="/employer/icon.png" alt="JobDozo" /></span>
          <span><strong>Job<em>Dozo</em></strong><small>Employer</small></span>
        </Link>

        <div className="company-switcher">
          <span className="avatar sm">{initials}</span>
          <div><strong>{user?.company || user?.name}</strong><small>{user?.verified ? "Verified Employer" : "Employer Account"}</small></div>
        </div>
